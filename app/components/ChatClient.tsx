...
export default function ChatClient() {
  ...
  useEffect(() => {
    if (mode && typeof window !== 'undefined') {
      import('simple-peer').then((SimplePeer) => {
        ...
        peer.on('data', (data: Uint8Array) => {
          ...
          setReceivedMessages(prev => [...prev, decodedMessage])
        })
  ...
  const handleConnect = () => {
    if (peerRef.current && remotePeerId) {
      console.log(JSON.parse(remotePeerId))
      peerRef.current.signal(JSON.parse(remotePeerId))
    }
  }
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-xl">
      <h2 className="text-xl font-bold mb-4">{mode === 'start' ? 'Your code:' : 'Enter code:'}</h2>
      {mode === 'start' ? (
