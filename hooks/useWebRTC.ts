...
export default function useWebRTC({ mode, setMode }: UseWebRTCProps) {
  ...
  const [receivedFiles, setReceivedFiles] = useState<{ id: string, name: string, url: string }[]>([]);
  ...
  const fileTransferQueue: { fileId: string, originalName: string, uniqueName: string, file: File }[] = [];
  ...
  const sendFileInChunks = async (fileInfo: { fileId: string, originalName: string, uniqueName: string, file: File }) => {
    ...
    try {
        ...
        while (retries < maxRetries) {
          try {
            ...
            if (retries >= maxRetries) {
              throw new Error(`Failed to send file ${fileInfo.originalName} after multiple retries`);
            }
      ...
      setReceivedMessages(prev => [...prev, `Me: Sent file - ${fileInfo.originalName} (${fileId})`]);
  ...
  const handleSendFile = (file: File) => {
    ...
    const existingFileNames = Array.from(fileTransfers.values()).map(transfer => transfer.originalName);
  ...
  const receiveFile = (fileInfo: { fileId: string, name: string, originalName: string, size: number, totalChunks: number }) => {
    setFileTransfers(prev => {
      const newTransfers = new Map(prev);
  ...
  const handleFileTransferComplete = (fileId: string) => {
    setFileTransfers(prev => {
      ...
      if (transfer) {
        ...
        setReceivedMessages(prev => [...prev, `Friend: File received - ${transfer.originalName} (${fileId})`]);
        setReceivedFiles(prev => [...prev, { id: fileId, name: transfer.originalName, url }]);
        transfer.status = 'completed';
    ...
    setFileTransfers(prev => {
      ...
      Array.from(newTransfers.entries()).forEach(([fileId, transfer]) => {
        if (transfer.status !== 'completed') {
          ...
          // Remove the file message from receivedMessages
          setReceivedMessages(prevMessages => 
            prevMessages.filter(msg => !msg.includes(`File received - ${transfer.fileName}`))
          );
          setReceivedMessages(prevMessages => 
            prevMessages.filter(msg => !msg.includes(`Sent file - ${transfer.fileName}`))
          );
        }
      });
      return newTransfers;
